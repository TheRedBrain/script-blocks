...
public class TriggeredBeaconBlockEntityRenderer implements BlockEntityRenderer<TriggeredBeaconBlockEntity> {
	public TriggeredBeaconBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
	}

	@Override
	public void render(TriggeredBeaconBlockEntity triggeredBeaconBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
		double o;
		double n;
		double m;
		double k;
		...
		BlockPos blockPos = triggeredBeaconBlockEntity.getAreaPositionOffset();
		Vec3i vec3i = triggeredBeaconBlockEntity.getAreaDimensions();
		...
		if (triggeredBeaconBlockEntity.showArea()) {
			WorldRenderer.drawBox(matrixStack, vertexConsumer, m, g, n, o, h, p, 0.9f, 0.9f, 0.9f, 1.0f, 0.5f, 0.5f, 0.5f);
			this.renderInvisibleBlocks(triggeredBeaconBlockEntity, vertexConsumer, blockPos, matrixStack);
		}
	}

	private void renderInvisibleBlocks(TriggeredBeaconBlockEntity entity, VertexConsumer vertices, BlockPos pos, MatrixStack matrices) {
	...
	@Override
	public boolean rendersOutsideBoundingBox(TriggeredBeaconBlockEntity triggeredBeaconBlockEntity) {
		return true;
	}
