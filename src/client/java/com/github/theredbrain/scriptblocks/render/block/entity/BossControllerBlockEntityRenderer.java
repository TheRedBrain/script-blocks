...
public class BossControllerBlockEntityRenderer implements BlockEntityRenderer<BossControllerBlockEntity> {
	public BossControllerBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
	}

	@Override
	public void render(BossControllerBlockEntity bossControllerBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
		double o;
		double n;
		double m;
		double k;
		...
		BlockPos blockPos = bossControllerBlockEntity.getAreaPositionOffset();
		Vec3i vec3i = bossControllerBlockEntity.getAreaDimensions();
		...
		if (bossControllerBlockEntity.showArea()) {
			WorldRenderer.drawBox(matrixStack, vertexConsumer, m, g, n, o, h, p, 0.9f, 0.9f, 0.9f, 1.0f, 0.5f, 0.5f, 0.5f);
			this.renderInvisibleBlocks(bossControllerBlockEntity, vertexConsumer, blockPos, matrixStack);
		}
	}

	private void renderInvisibleBlocks(BossControllerBlockEntity entity, VertexConsumer vertices, BlockPos pos, MatrixStack matrices) {
	...
	@Override
	public boolean rendersOutsideBoundingBox(BossControllerBlockEntity bossControllerBlockEntity) {
		return true;
	}
