...
import com.github.theredbrain.scriptblocks.ScriptBlocksClient;
...
public class TriggeredEntityRemoverBlockEntityRenderer implements BlockEntityRenderer<TriggeredEntityRemoverBlockEntity> {
	public TriggeredEntityRemoverBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
	}

	@Override
	public void render(TriggeredEntityRemoverBlockEntity triggeredEntityRemoverBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
		double o;
		double n;
		double m;
		double k;
		...
		Vec3i vec3i = triggeredEntityRemoverBlockEntity.getAreaDimensions();
		...
		if (triggeredEntityRemoverBlockEntity.showArea()) {
			WorldRenderer.drawBox(matrixStack, vertexConsumer, m, g, n, o, h, p, 0.9f, 0.9f, 0.9f, 1.0f, 0.5f, 0.5f, 0.5f);
			this.renderInvisibleBlocks(triggeredEntityRemoverBlockEntity, vertexConsumer, blockPos, matrixStack);
		}
	}

	private void renderInvisibleBlocks(TriggeredEntityRemoverBlockEntity entity, VertexConsumer vertices, BlockPos pos, MatrixStack matrices) {
	...
	@Override
	public boolean rendersOutsideBoundingBox(TriggeredEntityRemoverBlockEntity triggeredEntityRemoverBlockEntity) {
		return true;
	}
