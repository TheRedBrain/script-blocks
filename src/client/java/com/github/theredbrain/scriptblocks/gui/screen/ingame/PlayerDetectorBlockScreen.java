...
public class PlayerDetectorBlockScreen extends Screen {
	private static final Text HIDE_AREA_LABEL_TEXT = Text.translatable("gui.player_detector_block.hide_area_label");
	private static final Text SHOW_AREA_LABEL_TEXT = Text.translatable("gui.player_detector_block.show_area_label");
	...
	protected void init() {
		...
		this.addDrawableChild(CyclingButtonWidget.onOffBuilder(HIDE_AREA_LABEL_TEXT, SHOW_AREA_LABEL_TEXT).initially(this.showArea).omitKeyText().build(this.width / 2 - 154, 30, 300, 20, Text.empty(), (button, showArea) -> {
		...
		this.triggeredBlockPositionOffsetXField = new TextFieldWidget(this.textRenderer, this.width / 2 - 154, 135, 50, 20, Text.translatable(""));
		this.triggeredBlockPositionOffsetXField.setMaxLength(128);
		this.triggeredBlockPositionOffsetXField.setText(Integer.toString(this.playerDetectorBlock.getTriggeredBlock().getLeft().getX()));
		this.addSelectableChild(this.triggeredBlockPositionOffsetXField);
		this.triggeredBlockPositionOffsetYField = new TextFieldWidget(this.textRenderer, this.width / 2 - 100, 135, 50, 20, Text.translatable(""));
		this.triggeredBlockPositionOffsetYField.setMaxLength(128);
		this.triggeredBlockPositionOffsetYField.setText(Integer.toString(this.playerDetectorBlock.getTriggeredBlock().getLeft().getY()));
		this.addSelectableChild(this.triggeredBlockPositionOffsetYField);
		this.triggeredBlockPositionOffsetZField = new TextFieldWidget(this.textRenderer, this.width / 2 - 46, 135, 50, 20, Text.translatable(""));
		this.triggeredBlockPositionOffsetZField.setMaxLength(128);
		this.triggeredBlockPositionOffsetZField.setText(Integer.toString(this.playerDetectorBlock.getTriggeredBlock().getLeft().getZ()));
		this.addSelectableChild(this.triggeredBlockPositionOffsetZField);

		this.triggeredBlockResets = this.playerDetectorBlock.getTriggeredBlock().getRight();
		this.addDrawableChild(CyclingButtonWidget.onOffBuilder(Text.translatable("gui.triggered_block.toggle_triggered_block_resets_button_label.on"), Text.translatable("gui.triggered_block.toggle_triggered_block_resets_button_label.off")).initially(this.triggeredBlockResets).omitKeyText().build(this.width / 2 + 8, 135, 150, 20, Text.empty(), (button, triggeredBlockResets) -> {
	...
	public void resize(MinecraftClient client, int width, int height) {
		...
		boolean bool1 = this.triggeredBlockResets;
		...
		String string6 = this.triggeredBlockPositionOffsetXField.getText();
		String string7 = this.triggeredBlockPositionOffsetYField.getText();
		String string8 = this.triggeredBlockPositionOffsetZField.getText();
		this.init(client, width, height);
		this.showArea = bool;
		this.triggeredBlockResets = bool1;
		...
		this.triggeredBlockPositionOffsetXField.setText(string6);
		this.triggeredBlockPositionOffsetYField.setText(string7);
		this.triggeredBlockPositionOffsetZField.setText(string8);
	}
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		context.drawTextWithShadow(this.textRenderer, TRIGGERED_BLOCK_POSITION_TEXT, this.width / 2 - 153, 125, 0xA0A0A0);
