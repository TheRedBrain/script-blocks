...
public class TriggeredBeaconBlockScreen extends Screen {
	private static final Text TRIGGERED_TRUE_LABEL_TEXT = Text.translatable("gui.triggered_beacon_block.triggered_true_label");
	private static final Text TRIGGERED_FALSE_LABEL_TEXT = Text.translatable("gui.triggered_beacon_block.triggered_false_label");
	private static final Text HIDE_AREA_LABEL_TEXT = Text.translatable("gui.triggered_beacon_block.hide_area_label");
	private static final Text SHOW_AREA_LABEL_TEXT = Text.translatable("gui.triggered_beacon_block.show_area_label");
	private static final Text AREA_DIMENSIONS_LABEL_TEXT = Text.translatable("gui.triggered_beacon_block.area_dimensions_label");
	...
	private static final Text SHOW_PARTICLES_LABEL_TEXT = Text.translatable("gui.triggered_beacon_block.show_particles_label");
	private static final Text HIDE_ICON_LABEL_TEXT = Text.translatable("gui.triggered_beacon_block.hide_icon_label");
	private static final Text SHOW_ICON_LABEL_TEXT = Text.translatable("gui.triggered_beacon_block.show_icon_label");
	...
	private CyclingButtonWidget<TriggeredBeaconBlockEntity.TriggeredMode> cycleTriggeredModeButton;
	private boolean triggered;
	private boolean triggeredBlockResets;
	private TriggeredBeaconBlockEntity.TriggeredMode triggeredMode;

	public TriggeredBeaconBlockScreen(TriggeredBeaconBlockEntity triggeredBeaconBlock) {
		super(NarratorManager.EMPTY);
		this.triggeredBeaconBlock = triggeredBeaconBlock;
	...
	protected void init() {
		...
		this.areaDimensionsXField.setText(Integer.toString(this.triggeredBeaconBlock.getAreaDimensions().getX()));
		...
		this.areaDimensionsYField.setText(Integer.toString(this.triggeredBeaconBlock.getAreaDimensions().getY()));
		...
		this.areaDimensionsZField.setText(Integer.toString(this.triggeredBeaconBlock.getAreaDimensions().getZ()));
		...
		this.areaPositionOffsetXField.setText(Integer.toString(this.triggeredBeaconBlock.getAreaPositionOffset().getX()));
		...
		this.areaPositionOffsetYField.setText(Integer.toString(this.triggeredBeaconBlock.getAreaPositionOffset().getY()));
		...
		this.areaPositionOffsetZField.setText(Integer.toString(this.triggeredBeaconBlock.getAreaPositionOffset().getZ()));
		...
		this.toggleWasTriggeredButton = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(TRIGGERED_TRUE_LABEL_TEXT, TRIGGERED_FALSE_LABEL_TEXT).initially(this.triggered).omitKeyText().build(this.width / 2 - 154, 89, 300, 20, Text.empty(), (button, triggered) -> {
			this.triggered = triggered;
		}));

		this.triggeredMode = this.triggeredBeaconBlock.getTriggeredMode();
		this.cycleTriggeredModeButton = this.addDrawableChild(CyclingButtonWidget.builder(TriggeredBeaconBlockEntity.TriggeredMode::asText).values((TriggeredBeaconBlockEntity.TriggeredMode[]) TriggeredBeaconBlockEntity.TriggeredMode.values()).initially(this.triggeredMode).omitKeyText().build(this.width / 2 - 154, 113, 300, 20, Text.empty(), (button, triggeredMode) -> {
	...
	public void resize(MinecraftClient client, int width, int height) {
		...
		TriggeredBeaconBlockEntity.TriggeredMode var2 = this.triggeredMode;
	...
	private boolean updateAreaBlock() {
		ClientPlayNetworking.send(new UpdateTriggeredBeaconBlockPacket(
				this.triggeredBeaconBlock.getPos(),
				this.showArea,
				new Vec3i(
	...
	public static enum ScreenPage implements StringIdentifiable {
		...
		public Text asText() {
			return Text.translatable("gui.triggered_beacon_block.screenPage." + this.name);
		}
	}
}
